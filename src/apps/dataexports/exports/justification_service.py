...
class ExportJustificationService:
    ...
    def actuacions_rows_activities(self):
        ...
        for item in self.sessions_obj:
            ...
            if item.place is not None and item.place.town:
                town = self.export_manager.get_correlation("towns", item.place.town.name_for_justification)
            material_difusio = "No"
            if item.file1.name:
                material_difusio = "Sí"
        ...
        for project_id, project in self.stages_obj.items():
            for group_name, group in project.items():
                ...
                if item.project.town:
                    town = self.export_manager.get_correlation("towns", item.project.town.name_for_justification)
                circle = (
                     CirclesChoices(item.circle).label
                     if item.circle is not None
                     else ("", True)
    ...
    def actuacions_rows_founded_projects(self):
        ...
        for created_entity in self.created_entities:
            ...
            self.export_manager.row_number += 1
            town = (
                self.export_manager.get_correlation("towns", created_entity.project.town.name_for_justification)
        ...
        for project_id, project in self.stages_obj.items():
            for group_name, group in project.items():
                ...
                for participant in group['participants']:
                    ...
                    town = ("", True)
                    if participant.town:
                        town = self.export_manager.get_correlation("towns", participant.town.name_for_justification)
    ...
    def insercionslaborals_rows(self):
        ...
        for insertion in obj:
            ...
            if insertion.user.town:
                town = self.export_manager.get_correlation("towns", insertion.user.town.name_for_justification)
